...
let globals = {
    ...
    hmpoGetError(ctx, params, renderGroupedError = false) {
        ...
        if (fieldErrorGroup) {
            if (error && !renderGroupedError) return true;
            let errorGroupError = fieldErrorGroup && ctx('errors.' + fieldErrorGroup);
            if (!error && errorGroupError && !errorGroupError.errorGroup) return true;
        }
    ...
    hmpoGetErrorSummary(ctx) {
        ...
        let fieldErrors = ctx('errors') || {};
        let dateFields = (ctx('options.dateFields') || []).filter(fieldName =>
            ctx('options.fields.' + fieldName + '.showMultipleErrors') === true
        );
        let addError = error => {
            errorKeys.add(error.key);
            errorSummary.push({
        ...
        for (let error of errors) {
            if (!dateFields.includes(error.key)) {
                addError(error);
                continue;
            }

            for (let part of ['day', 'month', 'year']) {
                let key = error.key + '-' + part;
                let partError = error.field === key ? error : fieldErrors[key];
                if (partError && !errorKeys.has(partError.key)) addError(partError);
            }
        }

        for (let fieldName of dateFields) {
            let groupError = fieldErrors[fieldName];
            for (let part of ['day', 'month', 'year']) {
                let key = fieldName + '-' + part;
                let error = fieldErrors[key];
                if (!error || errorKeys.has(key) || (groupError && groupError.field === key)) continue;
                errorKeys.add(key);
                errorSummary.push({
                    href: '#' + (error.field || error.key),
                    text: globals.hmpoBuildErrorMessage(ctx, error, true)
