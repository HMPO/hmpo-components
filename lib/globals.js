...
let globals = {
    ...
    hmpoGetItems(ctx, params, value, required, setIdsBasedOnValues) {
        ...
        items = items.map(item => {
            ...
            if (item.label) {
                if (item.label.html) item.html = item.label.html;
                if (item.label.text) item.text = item.label.text;
                if (item.label.key) item.key = item.label.key;
            }
            ...
            if (conditional) item.conditional = typeof conditional === 'string' ? { html: conditional } : conditional;
            if (setIdsBasedOnValues && !item.id) {
                let cleanedValue = String(item.value).toLowerCase().replace(/[^a-zA-Z0-9]+/g, '');
