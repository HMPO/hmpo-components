...
**NOTE:**
`hmpo-components` **v10.x** is compatible with [GOV.UK Frontend v6.x](https://github.com/alphagov/govuk-frontend), which requires [ECMAScript Modules (ESM)](https://nodejs.org/api/esm.html) and a modern JavaScript bundler.
...
- This package is validated against GOV.UK Frontend `6.4+` (including `6.5.0`).
...
The `statuses` map keys correspond to session values stored via `setValuesOnSave`. When a task's `statusField` value matches a key in `statuses`, that status is rendered. The value in the map is a [GOV.UK tag colour](https://design-system.service.gov.uk/components/tag/) (e.g. `'blue'`, `'teal'`) or `false` for plain text with no tag.
